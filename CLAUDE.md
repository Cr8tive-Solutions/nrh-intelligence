...
`Customer::paymentMode()` returns `'cash'` or `'credit'` via `Customer::currentAgreement()` — the unexpired agreement with the latest expiry date, falling back to the most recent one (same rule as the admin portal's `Customer::activeAgreement()`; keep them in sync). `Agreement::isCashBilled()` reads from `config('billing.cash_aliases')`. Cash customers upload payment slips; credit customers are invoiced monthly.

### Cross-app storage & URL ids

- Report PDFs are generated by the **admin portal** onto its private disk; this app serves them through the `admin_local` disk (root `../nrh-admin/storage/app/private`, env `ADMIN_STORAGE_PATH` — **must be set in production** or report downloads 404). The admin portal reads this app's uploads (payment slips, receipts, candidate documents, consent evidence) through its mirror `client_local` disk.
- Hashid URL ids salt from `HASHIDS_SALT` (falls back to `APP_KEY` — see `config/hashids.php`). Always pass model instances (or `hid($id)`) to `route()` — a raw integer id 404s in the hashid binding.
