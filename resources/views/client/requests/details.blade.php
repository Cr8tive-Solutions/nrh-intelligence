<x-client.layouts.app pageTitle="Request {{ $request->reference }}">
    ...
    @php
        ...
        $heroState = match (true) {
            ...
            $request->status === 'prelim' => [
                'bg' => 'var(--ink-100)', 'accent' => 'var(--ink-900)', 'icon' => 'check',
                'title' => 'Preliminary report ready',
                'desc'  => 'Your preliminary report is available to download below. Full verification is still in progress — we\'ll notify you once the complete report is ready.',
            ],
        ...
        if ($isCashBilled) {
            $simpleSteps = [
                ...
                ['label' => 'Checking',  'sub' => $isComplete ? 'Completed' : 'Background verification',
                 'done'    => $isComplete,
                 'current' => in_array($request->status, ['in_progress', 'prelim']) || ($isCashPaymentVerified && $request->status === 'new'),
                 'flagged' => $isFlagged],
            ...
            $simpleSteps = [
                ...
                ['label' => 'Checking',  'sub' => $request->status === 'in_progress' ? 'In progress' : ($isComplete ? 'Completed' : 'Awaiting start'),
                 'done'    => $isComplete,
                 'current' => in_array($request->status, ['new', 'in_progress', 'prelim']),
                 'flagged' => $isFlagged],
                ['label' => 'Done',      'sub' => $isComplete ? $request->updated_at->format('d M Y') : 'Est. ' . $dueDate->format('d M'),
                 'done' => $isComplete, 'current' => false, 'flagged' => false],
            ];
